						...
						<div className="flex-1 space-y-4 overflow-y-auto px-4">
							...
							{hasVariants && (
								<div>
									...
									<Select
										value={variant || "__all__"}
										onValueChange={(v) => setVariant(v === "__all__" ? "" : v)}>
										<SelectTrigger className="mt-1 w-full">
...
function ProductFormDialog({
	...
	useEffect(() => {
		if (editProduct) {
			const variantRows: VariantRow[] = Array.isArray(editProduct.variants)
				? editProduct.variants.map((v) => ({
						variant: v.variant ?? "",
						qty: v.qty != null ? String(v.qty) : "0",
						skuCode: v.skuCode ?? "",
	...
	return (
		<Sheet open={open} onOpenChange={onOpenChange}>
			<SheetContent
				side="right"
				className="flex flex-col h-full w-full sm:max-w-3xl overflow-hidden">
				<SheetHeader>
				...
				<Form {...form}>
					<form
						...
						<div className="flex-1 min-h-0 overflow-y-auto px-4 space-y-4">
							...
							{/* Variants (when tracking inventory; create: editable, edit: read-only) */}
							{form.watch("isInventoryItem") && (
								<div className="space-y-2">
									<div className="flex items-center justify-between">
										<label className="text-sm font-medium">
											Variants {isEdit ? "" : "(optional)"}
										</label>
										{!isEdit && (
											<Button
												type="button"
									...
									<p className="text-xs text-muted-foreground">
										{isEdit
											? "Variant quantities can be changed via Stock adjust."
											: "Variant combo e.g. "}
										{!isEdit && (
											<>
												<code className="rounded bg-muted px-1">orange-M</code>{" "}
												and quantity. SKU is generated by the backend.
											</>
										)}
									</p>
